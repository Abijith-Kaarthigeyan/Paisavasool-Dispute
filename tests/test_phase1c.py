...
async def test_amendment_agent_success(mock_openrouter_amendment):
    ...
    with patch("httpx.AsyncClient.post", new_callable=AsyncMock, return_value=mock_openrouter_amendment):
        with patch("src.core.config.settings.settings.OPENROUTER_API_KEY", "sk-openrouter-key"):
...
async def test_reference_agent_success(mock_openrouter_reference):
    ...
    with patch("httpx.AsyncClient.post", new_callable=AsyncMock, return_value=mock_openrouter_reference):
        with patch("src.core.config.settings.settings.OPENROUTER_API_KEY", "sk-openrouter-key"):
            res = await PaymentReferenceExtractionAgent.extract_reference("I paid via UTR123456789")
...
async def test_reference_agent_regex_fallback():
    ...
    with patch("src.core.config.settings.settings.OPENROUTER_API_KEY", ""):
        with patch("src.core.config.settings.settings.GEMINI_API_KEY", ""):
            res = await PaymentReferenceExtractionAgent.extract_reference("Payment ref SBI1234567890123")
...
async def test_mail_agent_llm(mock_openrouter_mail):
    ...
    with patch("httpx.AsyncClient.post", new_callable=AsyncMock, return_value=mock_openrouter_mail):
        with patch("src.core.config.settings.settings.OPENROUTER_API_KEY", "sk-openrouter-key"):
...
async def test_collections_pausing(db_session: AsyncSession):
    ...
    case = await case_repo.create_case(case_number="CASE-2026-999010", customer_email="customer@test.com")
    ...
    with patch("src.data.clients.ar_service_client.ARServiceClient.pause_collections", new_callable=AsyncMock, return_value=mock_resp):
...
async def test_payment_checker_paid_invoice(db_session: AsyncSession):
    ...
    case = await case_repo.create_case(case_number="CASE-2026-999011", customer_email="customer@test.com")
    dispute = await dispute_repo.create_dispute(
    ...
    with patch("src.data.clients.ar_service_client.ARServiceClient.get_payment_status", new_callable=AsyncMock, return_value="PAID"):
...
async def test_payment_checker_missing_reference(db_session: AsyncSession):
    ...
    case = await case_repo.create_case(case_number="CASE-2026-999012", customer_email="customer@test.com")
    dispute = await dispute_repo.create_dispute(
    ...
    with patch("src.data.clients.ar_service_client.ARServiceClient.get_payment_status", new_callable=AsyncMock, return_value="UNPAID"):
        from src.core.workflow.graph import payment_checker_node
...
async def test_e2e_amendment_path(db_session: AsyncSession, seed_users):
    ...
    await db_session.execute(
        ...
        {"id": str(invoice_id) if is_sqlite else invoice_id, "cust_id": str(uuid4()), "batch_id": str(uuid4())}
    )
    await db_session.commit()
    ...
    case = await case_repo.create_case(case_number="CASE-2026-999015", customer_email="customer@test.com")
    dispute = await dispute_repo.create_dispute(
    ...
    mock_ar = MagicMock()
    mock_ar.get_invoice_details = AsyncMock(return_value={
        "invoice_number": "INV-AMEND-1",
        "subtotal_amount": 100,
        "tax_amount": 18,
        "total_amount": 118,
    ...
    with patch("src.core.workflow.graph.ARServiceClient", return_value=mock_ar):
        with patch("src.core.workflow.amendment_agent.AmendmentResolutionAgent.resolve_amendment", new_callable=AsyncMock, return_value=mock_amend_outcome):
            with patch("src.core.workflow.mail_agent.DisputeMailAgent.generate_mail", new_callable=AsyncMock, return_value=mock_mail_outcome):
