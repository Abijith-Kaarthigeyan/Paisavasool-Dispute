...
class PaymentReferenceExtractionAgent:
    ...
    def _regex_fallback(cls, raw_customer_text: str) -> Dict[str, Any]:
        ...
        if utr_match:
            return {
                "reference_number": utr_match.group(0).upper(),
                "confidence": 85.0
            }
            
        # Try generic reference pattern if found
        tx_match = re.search(r"(?:tx|txn|ref|reference|utr)[:\-\s#]+([A-Z0-9]+)", raw_customer_text, re.IGNORECASE)
        if tx_match:
            return {
                "reference_number": tx_match.group(1).upper(),
                "confidence": 80.0
            }
...
"""
        ...
        if openrouter_key and not openrouter_key.startswith("mock-") and openrouter_key.strip():
            for model, provider in models_to_try:
                ...
                try:
                    logger.info("Attempting payment reference extraction with %s model: %s", provider, model)
                    headers = {
                    ...
                    logger.info("Payment reference extraction succeeded with model: %s", model)
                    ...
                    logger.warning("Payment reference extraction failed with model %s: %s", model, str(e))
        ...
        if gemini_key and not gemini_key.startswith("mock-") and gemini_key.strip():
            ...
            try:
                logger.info("Attempting payment reference extraction with direct Gemini model: %s", model)
                ...
                logger.error("Direct Gemini payment reference extraction failed: %s", str(e))
