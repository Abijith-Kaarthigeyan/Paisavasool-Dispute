...
async def create_case_intake(
    ...
    if message_id:
        raw_res = await db.execute(
            text(
                "SELECT id, case_number FROM dispute_cases WHERE original_message_id = :msg_id AND is_deleted = false"
            ),
            {"msg_id": message_id},
        )
        row = raw_res.first()
        if row:
            ...
            disp_res = await db.execute(
                text(
                    "SELECT id FROM disputes WHERE case_id = :c_id AND is_deleted = false"
                ),
                {"c_id": case_id},
            )
            ...
            for d_id in dispute_ids:
                await db.execute(
                    ...
                    ),
                    {
                        "act_id": uuid4(),
                        "d_id": d_id,
                        "meta": '{"info": "Intake email with message_id was resubmitted and bypassed"}',
