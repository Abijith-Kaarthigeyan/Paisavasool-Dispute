...
class ARServiceClient:
    ...
    async def get_invoice(self, invoice_id: UUID, custom_token: str | None = None) -> dict:
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if resp.status_code == 404:
                    raise ARServiceClientException(f"Invoice {invoice_id} not found in AR.", status_code=404)
                resp.raise_for_status()
                return resp.json()
        except httpx.HTTPStatusError as e:
            raise ARServiceClientException(f"AR Service HTTP error: {e.response.text}", status_code=e.response.status_code)
        except Exception as e:
            raise ARServiceClientException(f"AR Service communication error: {str(e)}")

    async def get_invoice_details(self, invoice_id: UUID, custom_token: str | None = None) -> dict:
    ...
    async def get_customer(self, customer_id: UUID, custom_token: str | None = None) -> dict:
        ...
        try:
            async with httpx.AsyncClient() as client:
                resp = await client.get(url, headers=headers, timeout=5.0)
                if resp.status_code == 404:
                    raise ARServiceClientException(f"Customer {customer_id} not found in AR.", status_code=404)
                resp.raise_for_status()
                return resp.json()
        except httpx.HTTPStatusError as e:
            raise ARServiceClientException(f"AR Service HTTP error: {e.response.text}", status_code=e.response.status_code)
    ...
    async def get_payment_status(self, invoice_id: UUID, custom_token: str | None = None) -> str:
    ...
    async def get_collection_case(self, case_id: UUID, custom_token: str | None = None) -> dict:
        ...
        try:
            async with httpx.AsyncClient() as client:
                resp = await client.get(url, headers=headers, timeout=5.0)
                if resp.status_code == 404:
                    raise ARServiceClientException(f"Collection case {case_id} not found in AR.", status_code=404)
                resp.raise_for_status()
                return resp.json()
        except httpx.HTTPStatusError as e:
            raise ARServiceClientException(f"AR Service HTTP error: {e.response.text}", status_code=e.response.status_code)
    ...
    async def pause_collections(self, invoice_id: UUID, custom_token: str | None = None) -> dict:
    ...
    async def resume_collections(self, invoice_id: UUID, custom_token: str | None = None) -> dict:
    ...
    async def find_payment_reference(self, reference_number: str, custom_token: str | None = None) -> str:
